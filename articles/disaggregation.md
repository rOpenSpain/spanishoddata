...
We’ll start by loading a week’s worth of origin-destination data for the
city of Salamanca, building on the example in the README (note: these
...
The results show that you can add value to the OD data by disaggregating
the desire lines with the `{odjitter}` package. This can be useful for
understanding the spatial distribution of trips within a zone for
transport planning.

We have plotted the disaggregated desire lines on top of the major road
network in Salamanca. A next step could be routing to help prioritise
