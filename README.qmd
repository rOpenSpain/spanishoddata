...
```{r}
dates <- seq(as.Date("2024-03-01"), as.Date("2024-03-07"), by = "day")
od_db <- spod_get_od(zones = "distritos", dates = dates)
# TODO: debug error message
# > od_db <- spod_get_od(zones = "distritos", dates = dates)
# Error: rapi_prepare: Failed to prepare query CREATE VIEW od_csv_raw AS SELECT *
# /* csv_folder needs to be replaced with a valid path
# in R use glue::glue() */
# FROM read_csv_auto('/data/bronze/spanishoddata/raw_data_cache/v2//estudios_basicos/por-distritos/viajes/ficheros-diarios/**/*.csv.gz',
#     delim='|',
#     header=TRUE,
...
# },
# dateformat='%Y%m%d');
# Error: IO Error: No files found that match the pattern "/data/bronze/spanishoddata/raw_data_cache/v2//estudios_basicos/
```
