...
```{r}
districts <- spod_get_zones(zones = "dist", ver = 1, quiet = TRUE)
```
...
```{r}
municipalities <- spod_get_zones(zones = "muni", ver = 1, quiet = TRUE)
```
