...
```{r}
od_dist_1 <- spod_get_od_v1(zones = "dist",
...
od_dist_1_lazy <- od_dist_1 |>
  ...
  summarise(total_day_trips = sum(n_trips, na.rm = TRUE),
    .groups = "drop") |>
  group_by(id_origin, id_destination) |>
  summarise(mean_dayly_trips = mean(total_day_trips, na.rm = TRUE),
    .groups = "drop")
```
...
```{r}
od_muni_1 <- spod_get_od_v1(zones = "muni",
