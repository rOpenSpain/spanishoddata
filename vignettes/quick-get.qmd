...
::: callout-note
Setting a local data directory in this case is optional, as the data is downloaded directly from API and there is no caching on disk. However, the metadata will be downloaded to check the range of valid dates available at the time of the request. So metadata will be downloaded to a temporary location by default, or to the data directory, if you do set it.
:::
...
You can now proceed to analyse these flows or visualise them as in the [static](flowmaps-static.html) and [interactive](flowmaps-interactive.html) flow maps tutorials.
