...
**Converting the data to either `DuckDB` or `Parquet` will improve the computation time up to 10x compared with using data frames returned with `spod_get()`, which reads CSV files every time you call it.**
