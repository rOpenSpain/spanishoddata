---
title: "Making interactive flow maps with `mapgl`"
vignette: >
  %\VignetteIndexEntry{Making interactive flow maps with `mapgl`}
...
```{r}
# remotes::install_github("e-kotov/mapgl@flow")
```
...
To visualise the flows, `{mapgl}` expects a `data.frame` with flows and a `data.frame` with location coordinates or an `sf` POINTS object.

**Locations `data.frame` example**
...
# A tibble: 6 × 3
  origin   dest       count
  <fct>    <fct>      <dbl>
1 01001_AM 26026_AM   26.5 
2 01001_AM 28901_AM    5.25
3 01001_AM 4801306    10.4 
4 01001_AM 5029709    23.1 
5 01002    01036    2716.  
6 01002    0105903    21.7 
```
...
Geometry type: POINT
Dimension:     XY
Bounding box:  xmin: -5.555105 ymin: 37.74344 xmax: 2.854264 ymax: 42.59849
Geodetic CRS:  WGS 84
# A tibble: 6 × 2
  id                       geom
  <chr>             <POINT [°]>
1 2408910  (-5.555105 42.59849)
2 22117_AM (0.3260681 42.17266)
3 2305009  (-3.813645 37.74344)
4 07058_AM  (2.854264 39.80672)
5 2305006  (-3.822951 37.77294)
6 2305005   (-3.81511 37.86309)
```
...
```{r}
flowmap <- maplibre(bounds = locations) |>
  add_flowmap(
    id = "flows",
    locations = locations,
    flows = flows,
    flow_dim_basemap = TRUE,
    flow_blend_mode = "screen",
    flow_show_settings = TRUE
  )
