...
**TL;DR: For analysing more than 1 week of data, use `spod_convert()` to convert the data into `DuckDB` and `spod_connect()` to conenct to it for analysis using `{dplyr}`. Skip to the [section about it](#duckdb).**

Using origin-destination data as an example, this vignette shows how to download the mobility datasets from versions v1 (2020-2021) and v2 (2022-onwards) of the Spanish mobility data and convert them into analysis-ready formats. See description of datasets in the [Codebook and cookbook for v1 (2020-2021) Spanish mobility data](v1-2020-2021-mitma-data-codebook.qmd) and in the [Codebook and cookbook for v2 (2022 onwards) Spanish mobility data](v2-2022-onwards-mitma-data-codebook.qmd).
...
2. as a connection to DuckDB or Parquet files on disk with `spod_convert()` + `spod_connect()`. The latter is recommended for large datasets (more than several days), as it is much faster and more memory efficient, as we demonstarte below.

`spod_get()` returns objects that are only appropriate for small datasets representing a few days of the national origin-destination flows. We recommend converting the data into analysis-ready formats (`DuckDB` or `Parquet`), allowing you to work with much longer time periods (months and years) on a consumer laptop (with 8-16 GB of memory).

**Converting the data to either `DuckDB` or `Parquet` can lead to ~10x speed-up compared to using data frames returned by `spod_get()`, which have to read the original compressed CSV.gz files from disk every time you filter, aggregate or reshape the data.**
...
In our testing (see @fig-csv-duckdb-parquet-speed), using `DuckDB` database has a significant speed advantage over running the analysis on raw CSV.gz files. Using the full origin destination dataset for 18 months, we compared a query that would find mean hourly number of trips between all zone pairs over the whole time period. `DuckDB` database was up to 8 times faster than running the analysis on raw CSV.gz files, and up to 5 times faster than running the analysis on `Parquet` files.
...
The approaches to working with all formats are exactly the same, as under the hood all analysis is actually done by the `DuckDB` [@duckdb-r] engine. So from that point of view, there is no difference between data formats. You can manipulate the data using `{dplyr}` functions such as `select()`, `filter()`, `mutate()`, `group_by()`, `summarise()`, etc. In the end of any sequence of commands you will need to add `collect()` to execute the whole chain of data manipulations and load the results into memory in an R `data.frame`/`tibble`. We provide examples in the following sections.

### Use cases

The choice between converting to `DuckDB` and `Parquet` could also be made based on your needs:
...
Let's start by loading the package and setting up the data directory, then we can explore how the functions for downloading and converting the dataset work.
...
As you might have seen in the codebooks for [v1](v1-2020-2021-mitma-data-codebook.qmd) and [v2](v2-2022-onwards-mitma-data-codebook.qmd) data, you can get a single day's worth of data as an in-memory object with `spod_get()`:
...
# Analysing the data using `Parquet` {#arrow-parquet}
