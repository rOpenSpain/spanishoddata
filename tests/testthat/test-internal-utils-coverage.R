...
test_that("spod_infer_data_v_from_dates handles ignore_missing_dates", {
  ...
  expect_error(
    spanishoddata:::spod_infer_data_v_from_dates(as.Date("2030-01-01")),
    "All requested dates are missing"
  )

  # Case 2: Some missing (default error)
  expect_error(
    spanishoddata:::spod_infer_data_v_from_dates(as.Date(c("2020-01-01", "2030-01-01"))),
    "Some dates do not match"
  )

  # Case 3: Some missing (ignore = TRUE)
  res <- spanishoddata:::spod_infer_data_v_from_dates(
  ...
  res_null <- spanishoddata:::spod_infer_data_v_from_dates(
    as.Date("2030-01-01"),
    ignore_missing_dates = TRUE
  )
  ...
  expect_equal(
    spanishoddata:::spod_infer_data_v_from_dates("cached_v1"), 1
  )
  expect_equal(
    spanishoddata:::spod_infer_data_v_from_dates("cached_v2"), 2
  )
})

test_that("spod_graphql_valid_dates works (mocked)", {
  # Clear memoise cache to ensure we test the request logic
  memoise::forget(spanishoddata:::spod_get_hmac_secret_memoised)
    ...
    req_perform = function(...) {
      structure(
        list(
          ...
          status_code = 200, 
          headers = list(), 
          body = charToRaw('<html><body><script>var import_meta_env = {"HMAC_SECRET": "1234567890"};</script></body></html>')
        ),
  ...
  local_mocked_bindings(
    resp_body_json = function(...) {
      list(data = list(dates_find_available_dates_basic = data.frame(
  ...
  dates <- spanishoddata:::spod_graphql_valid_dates()
...
test_that("spod_assert_package handles missing package", {
  ...
  expect_message(
    expect_error(
      spanishoddata:::spod_assert_package(pkg_name)
...
test_that("spod_match_data_type handles NULL/Warning", {
  expect_error(spanishoddata:::spod_match_data_type("invalid_type"), "should be one of")
  expect_error(spanishoddata:::spod_match_data_type_for_local_folders("invalid_type", 1), "should be one of")
})
