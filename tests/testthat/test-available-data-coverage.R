...
test_that("spod_available_data falls back to XML on S3 error (v1)", {
  ...
  real_data <- readRDS(system.file("extdata", "available_data_v1.rds", package = "spanishoddata"))
  ...
  msgs <- capture_messages(
    capture.output(res <- spod_available_data(ver = 1, quiet = FALSE), file = NULL)
  )
...
test_that("spod_get_latest_v1_file_list handles download failure", {
  ...
  expect_error(
    spanishoddata:::spod_get_latest_v1_file_list(data_dir = test_dir, quiet = TRUE),
    "Network error"
  )
})
...
test_that("spod_get_latest_v2_file_list handles download failure", {
  ...
  expect_error(
    spanishoddata:::spod_get_latest_v2_file_list(data_dir = test_dir, quiet = TRUE),
    "Network error"
  )
})
...
test_that("spod_available_data_v2 handles file size imputation with categories", {
  ...
  msgs <- capture_messages(
    capture.output(res <- spanishoddata:::spod_available_data_v2(data_dir = test_dir, quiet = FALSE, use_s3 = TRUE), file = NULL)
  )
  expect_match(msgs, "S3 fetch failed", all = FALSE)
...
test_that("spod_available_data with use_s3=FALSE uses XML directly", {
  ...
  res <- spanishoddata:::spod_available_data_v2(
    data_dir = test_dir, 
    use_s3 = FALSE,
    quiet = TRUE
  )
...
test_that("read_data_links_xml with force=TRUE re-downloads", {
  test_dir <- withr::local_tempdir()
  metadata_folder <- file.path(test_dir, spanishoddata:::spod_subfolder_metadata_cache())
  dir.create(metadata_folder, recursive = TRUE)
  
  # Create an old XML file
  old_xml <- file.path(metadata_folder, paste0("data_links_v1_", Sys.Date() - 1, ".xml"))
  writeLines("<xml><link>test</link><pubDate>Mon, 14 Feb 2020 00:00:00 +0000</pubDate></xml>", old_xml)
  
  # Mock the download function
  download_called <- FALSE
  mock_download <- function(...) {
    download_called <<- TRUE
    new_xml <- file.path(metadata_folder, paste0("data_links_v1_", Sys.Date(), ".xml"))
    writeLines("<xml><link>new</link><pubDate>Mon, 14 Feb 2020 00:00:00 +0000</pubDate></xml>", new_xml)
    return(new_xml)
  }
  
  res <- spanishoddata:::read_data_links_xml(
...
test_that("read_data_links_xml uses cached file when not stale", {
  ...
  writeLines("<xml><link>cached</link><pubDate>Mon, 14 Feb 2020 00:00:00 +0000</pubDate></xml>", fresh_xml)
...
test_that("spod_available_data v2 falls back to XML on S3 error", {
  ...
  testthat::local_mocked_bindings(
    ...
    spod_get_latest_v2_file_list = function(...) {
      ...
      writeLines("<rss><channel><item><link>https://test.com/file.csv.gz</link><pubDate>Mon, 01 Jan 2024 00:00:00 +0000</pubDate></item></channel></rss>", xml_file)
      return(xml_file)
    }
  )
  ...
  msgs <- capture_messages(
    capture.output(res <- spod_available_data(ver = 2, use_s3 = TRUE, quiet = FALSE), file = NULL)
  )
  expect_match(msgs, "S3 fetch failed", all = FALSE)
