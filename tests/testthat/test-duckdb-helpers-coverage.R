...
test_that("spod_duckdb_od handles gau zones in v1", {
  expect_error(
    spod_duckdb_od(zones = "gau", ver = 1),
    "not available in v1 data"
  )
})

test_that("spod_duckdb_number_of_trips handles gau zones in v1", {
  expect_error(
    spod_duckdb_number_of_trips(zones = "gau", ver = 1),
    "not available in v1 data"
  )
})
...
test_that("spod_duckdb_number_of_trips works with real fixtures (v2 dist)", {
  ...
  nt_path <- file.path(
    test_dir,
    "raw_data_cache/v2/estudios_basicos/por-distritos/personas/ficheros-diarios/year=2022/month=2/day=1"
  )
...
test_that("spod_duckdb_overnight_stays works with real fixtures (v2 dist)", {
  ...
  os_path <- file.path(
    test_dir,
    "raw_data_cache/v2/estudios_basicos/por-distritos/pernoctaciones/ficheros-diarios/year=2022/month=2/day=1"
  )
...
test_that("spod_duckdb_filter_by_dates logic works with real SQL generation", {
  ...
  DBI::dbExecute(
    con,
    "CREATE VIEW raw_view AS SELECT 2020 as year, 1 as month, 1 as day, 'a' as val"
  )
  ...
  count_empty <- DBI::dbGetQuery(
    con,
    "SELECT count(*) as n FROM filtered_view_empty"
  )$n
...
test_that("spod_sql_where_dates generates correct WHERE clause", {
  ...
  res <- spod_sql_where_dates(dates)
