...
test_that("spod_get returns DuckDB connection with cached data", {
  ...
  orig_valid_dates <- spanishoddata:::spod_get_valid_dates
  mock_valid_dates <- function(ver, ...) {
    if (ver == 2) return(as.Date("2022-02-01"))
    return(as.Date(character(0)))
  }
  
  assignInNamespace("spod_get_valid_dates", mock_valid_dates, ns = "spanishoddata")
  on.exit({
    assignInNamespace("spod_get_valid_dates", orig_valid_dates, ns = "spanishoddata")
  }, add = TRUE)
  ...
  mock_func <- function(...) {
    ...
    if (length(files) == 0) stop("Mock metadata not found in test dir")
    readRDS(files[1])
  }
  
  # Save original and mock
  orig_func <- get("spod_available_data_v2", envir = asNamespace("spanishoddata"))
  ...
  withr::defer(assignInNamespace("spod_available_data_v2", orig_func, ns = "spanishoddata"))
