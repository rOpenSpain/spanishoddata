...
spod_convert <- function(
  ...
  if( length(save_format) > 1 ) {
    stop("Invalid save_format. Please select 'duckdb' or 'parquet', or leave empty to use the default, which is 'duckdb'.")
  }
  ...
  if (!save_format %in% c("duckdb", "parquet")) {
    stop("Invalid save_format. Please set `save_format` to 'duckdb' or 'parquet', or leave empty to use the default, which is 'duckdb'.")
  }

  # check for inconsistencies between save_format and save_path
  if(!is.null(save_format) & !is.null(save_path)) {
    if (!grepl("duckdb$", save_path) & save_format == "duckdb") {
      stop("`save_path` is set to a folder, while `save_format` is set to 'duckdb'. Please make sure that `save_format` also ends with '.duckdb' when setting format to 'duckdb', or leave `save_format` empty, so that it is inferred from `save_path`. Perhaps you wanted to save to `parquet` files? Then please set `save_format` to 'parquet', or leave `save_format` empty.")
    }  
  }
  ...
  if (overwrite == "update" & save_format == "duckdb") {
    stop("You are trying to save to a duckdb file with `overwrite = 'update'`. This is not supported. Please set `overwrite=TRUE` to remove the existing duckdb file and create a new one with the requested data.")
  }
