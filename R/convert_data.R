#' Convert data to DuckDB for faster analysis
#'
#' @param save_path The path save the DuckDB database file. If `NULL`, uses the default location in `data_dir` (set by the `SPANISH_OD_DATA_DIR` environment variable). Therefore, the default relative path is `<data_dir>/clean_data/v1/tabular/duckdb/<type>_<zones>.duckdb`.
...
#'
#' @description
#' Converts data for faster analysis into either DuckDB file or into parquet files in a hive-style directory structure.
#' 
#' 
#' @param save_path The path save the DuckDB database file or the path to a folder to save the parquet files.
#' 
#' * If `save_path` ends with `.duckdb`, it will be saved as a DuckDB database file.
#' 
...
spod_convert_for_analysis <- function(
  ...
  # check if save_path ends with .duckdb
  ...
  } if else (!grepl('\\.[a-zA-Z0-9]+$', save_path)){
    save_format <- "parquet"
    save_dir <- save_path
  ...
  if (isFALSE(quiet)){
    ...
    message(glue::glue("Converting cached v{ver} {type} data for {zones} to {save_format}: {save_path} \n... This may take some time. You can see if the process is still running by going to the {save_dir} folder and refreshing the folder content and checking if the file/folder size of {basename(save_path)} is increasing after each refresh. The conversion speed depends on your processor speed, number of cores and the speed of your disk storage."))
  }
  # add some indication on how long it may take from empirical experimentation
  # hopefully, the progress_bar feature will be implemented in duckdb R package soon, bug filed here https://github.com/duckdb/duckdb-r/issues/199
  # if I change the code to import data in batches it slows down significantly
  ...
  if (save_format == "parquet") {
    if (overwrite == FALSE){
      ...
      if (length(parquet_files) > 0) {
        message("You have set `overwrite = FALSE` and there are already parquet files in ", save_path)
        response <- readline("What should be done? [D]elete all existing files? [U]date the folder with any new data while keeping the existing files? [C]ancel? ")
        if (tolower(response) %in% c("d", "delete")) {
          fs::dir_delete(save_path)
        } else if (tolower(response) %in% c("u", "update")) {
          fs::file_delete(parquet_files)
          fs::dir_create(save_path)
        } else {
          message("Exiting without overwriting existing parquet files. You may delete them from ", save_path, " manually and rerun the function. Or rerun the function with `overwrite = TRUE`.")
      }
    }
  ...
  # a few instructions on how to use the duckdb file
  if (isFALSE(quiet)){
    message("You can start working with the imported data by runining:\n mydata <- spod_connect_converted_data('path=", save_path, "')")
    message("You can then manipulate `mydata` using `dplyr` functions such as `select()`, `filter()`, `mutate()`, `group_by()`, `summarise()`, etc. In the end of any sequence of commands you will need to add `collect()` to execute the whole chain and load the results into memory in an R `data.frame`/`tibble`")
    if (save_format == "duckdb"){
      message("For more in-depth usage of such tables, please refer to DuckDB documentation and examples at https://duckdb.org/docs/api/r#dbplyr . Some more useful examples can be found here https://arrow-user2022.netlify.app/data-wrangling#combining-arrow-with-duckdb .")
    } else if (save_format == "parquet"){
      message("For more in-depth usage of such tables, please refer to DuckDB documentation and examples at https://duckdb.org/docs/api/r#dbplyr . Some more useful examples can be found here https://arrow-user2022.netlify.app/data-wrangling#combining-arrow-with-duckdb . You may also use `arrow` package to work with parquet files https://arrow.apache.org/docs/r/ .")
    }
  }
