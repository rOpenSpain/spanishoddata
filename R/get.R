...
spod_get <- function(
  ...
  if (!cached_data_requested) {
    ...
    selected_data <- available_data |>
      dplyr::filter(.data$data_ymd %in% dates) |>
      dplyr::filter(grepl(
        "maestra1-mitma-distritos/ficheros-diarios",
        .data$local_path
      ))

    selected_paths <- selected_data$local_path
    rel_paths <- stringr::str_replace(selected_paths, data_dir, "")
    tmp_root <- fs::dir_create(file.path(
      tempdir(),
      "spod_subset",
      as.character(Sys.getpid())
    ))
    temp_paths <- file.path(tmp_root, rel_paths)
    ...
    dbExecute(
      con,
      "CREATE OR REPLACE VIEW od AS SELECT * FROM read_csv_auto('/private/var/folders/gb/t5zr5rn15sldqybrmqbyh6y80000gn/T/RtmpdEHdOz/spod_subset/76696/raw_data_cache/v1/maestra1-mitma-distritos/ficheros-diarios/**/*.csv.gz')"
    )
    dbListTables(con)
    dplyr::tbl(con, "od")
  }
