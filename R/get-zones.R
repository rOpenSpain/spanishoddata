...
spod_get_zones_v1 <- function(
  ...
  if (fs::file_exists(expected_gpkg_path)) {
    if (isFALSE(quiet)) {
      message(
        "Loading .gpkg file that already exists in data dir: ",
        expected_gpkg_path
      )
    }
...
spod_clean_zones_v1 <- function(zones_path, zones, quiet = FALSE) {
  ...
  relations_districts_col_names <- gsub(
    "distrito",
    "district",
    relations_districts_col_names
  )
  relations_districts_col_names <- gsub(
    "municipio",
    "municipality",
  ...
  relations_municipalities_col_names <- gsub(
    "municipio",
    "municipality",
    relations_municipalities_col_names
  )
  ...
  names(relations_municipalities_aggregated)[
    names(relations_municipalities_aggregated) == "municipality_mitma"
  ] <- "id"
  ...
  names(relations_districts_aggregated)[
    names(relations_districts_aggregated) == "district_mitma"
  ] <- "id"
  ...
  names(v2_v_1ref)[names(v2_v_1ref) == "names_in_v2_data"] <- glue::glue(
    "{eng_zones}_names_in_v2"
  )
  names(v2_v_1ref)[names(v2_v_1ref) == "ids_in_v2_data"] <- glue::glue(
    "{eng_zones}_ids_in_v2"
  )
...
spod_download_zones_v1 <- function(
  ...
  relation_files <- metadata[
    grepl("relaciones_(distrito|municipio)_mitma.csv", metadata$target_url),
  ]
  if (any(!fs::file_exists(relation_files$local_path))) {
    fs::dir_create(
      unique(fs::path_dir(relation_files$local_path)),
      recurse = TRUE
    )
...
spod_get_zones_v2 <- function(
  ...
  zones_regex <- glue::glue(
    "(zonificacion_{zones}\\.*)|(poblacion\\.csv)|(relacion_ine_zonificacionMitma\\.csv)"
  )
  sel_zones <- stringr::str_detect(metadata$local_path, zones_regex)
  metadata_zones <- metadata[sel_zones, ]
  metadata_zones_for_download <- metadata_zones[
    metadata_zones$downloaded == FALSE,
  ]
...
spod_clean_zones_v2 <- function(zones_path) {
  ...
  if (zones != "gau") {
    ...
    names(v2_v_1ref)[names(v2_v_1ref) == "ids_in_v1_data"] <- glue::glue(
      "{eng_zones}_ids_in_v1"
    )
